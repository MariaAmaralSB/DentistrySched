      ...
      <header className="h-14 border-b bg-white">
        <div className="max-w-6xl mx-auto h-full flex items-center justify-between px-4">
          ...
          <nav className="hidden sm:flex items-center gap-4 text-sm text-gray-600">
            <a className="hover:text-gray-900" href="/" onClick={(e)=>{e.preventDefault(); window.location.href="/";}}>Site</a>
            <a className="hover:text-gray-900" target="_blank" href={import.meta.env.VITE_API + "/swagger"}>Swagger</a>
          </nav>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-[220px_1fr] gap-6">
        ...
        <aside className="bg-white rounded-2xl shadow p-3 h-max">
          ...
          <div className="flex flex-col gap-1">
            ...
            {/* Reservado para próximos itens */}
            {/* <NavLink to="/admin/procedimentos" className={({isActive}) => `${navItem} ${isActive ? active : ""}`}>🧪 Procedimentos</NavLink> */}
            {/* <NavLink to="/admin/agenda" className={({isActive}) => `${navItem} ${isActive ? active : ""}`}>📅 Regras de Agenda</NavLink> */}
          </div>
        </aside>
