...
export default function Booking() {
  ...
  const [loadingProceds, setLoadingProceds] = useState(false);
  ...
  useEffect(() => {
    ...
    setLoadingProceds(true);
    PublicAPI.procedimentos(dentistaId)
      .then((list) => {
        setProceds(list);
        if (list.every(p => p.id !== procedimentoId)) {
          setProcedimentoId("");
        }
      })
      .catch((e) => console.error("Erro ao carregar procedimentos", e))
      .finally(() => setLoadingProceds(false));
  }, [dentistaId]);
        ...
        <div className="bg-white rounded-2xl shadow p-6 space-y-6">
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              ...
              >
                <option value="">{loadingDentistas ? "Carregando..." : "Selecione"}</option>
                {dentistas.map(d => (
            ...
            <div>
              ...
              <select
                ...
                disabled={!dentistaId || loadingProceds}
              >
                <option value="">
                  {!dentistaId ? "Selecione o dentista primeiro" :
                   loadingProceds ? "Carregando..." : "Selecione"}
                </option>
                {proceds.map(p => (
            ...
            <div>
              ...
              <input
                ...
                disabled={!dentistaId || !procedimentoId}
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {!podeBuscar && (
              <div className="text-sm text-gray-600">
                Selecione dentista, procedimento e data.
              </div>
            )}
